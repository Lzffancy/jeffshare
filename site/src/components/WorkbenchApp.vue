  ...
  <template v-if="tab === 'upload'">
    ...
    <div v-if="authChecked && !authed" class="card bg-base-100 shadow-md">
      <div class="card-body text-center py-10">
        ...
        <div class="card-actions justify-center mt-4">
          <a :href="`/login?redirect=/workbench/`" class="btn btn-primary">
            <svg class="w-4 h-4 mr-1" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2a5 5 0 0 1 5 5v3h1a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2v-8a2 2 0 0 1 2-2h1V7a5 5 0 0 1 5-5zm0 2a3 3 0 0 0-3 3v3h6V7a3 3 0 0 0-3-3z"/></svg>
            使用密码登录
          </a>
        </div>
...
onMounted(async () => {
  ...
  try {
    ...
    if (resp.ok) {
      ...
      authed.value = true;
      userLogin.value = data.user?.login || "admin";
    }
  } catch {
...
function doLogout() {
  document.cookie = "jeff_token=; max-age=0; path=/";
  authed.value = false;
  userLogin.value = "";
}
