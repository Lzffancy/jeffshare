"""Presentation routes — FastAPI API 路由

依赖注入：通过依赖函数组装 infrastructure 实现并注入到 application use case。
"""
...
from app.application.agent.dtos import SummarizeInput, SaveMarkdownInput
from app.application.agent.services import SummarizeUseCase, SaveMarkdownUseCase
from app.infrastructure.persistence.repositories import SqliteAgentConfigRepo
from app.infrastructure.llm.openai_client import OpenAIClient
from app.infrastructure.workflow.langgraph_engine import LangGraphWorkflowEngine
...
# ── 依赖（懒加载单例） ─────────────────────────────────────────────
# 模块级创建避免每次请求重复初始化，但 lazy 避免未设 key 时 import 炸
